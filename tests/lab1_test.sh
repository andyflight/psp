...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" localhost:8080/actuator/health)
...
if ! docker logs psp-app-1 --tail 5 | jq . > /dev/null 2>&1; then
...
sleep 3
STATUS_503=$(curl -s -o /dev/null -w "%{http_code}" localhost:8080/actuator/health)
